  ...
  return (
    ...
    >
      ...
      <div className="p-5 flex flex-col flex-1">
          ...
          <div className="flex items-center space-x-2.5">
            ...
            <div>
              <div className="flex items-center space-x-1.5">
                <PosterIcon className={`w-3.5 h-3.5 shrink-0 ${ch.posterType === 'government' ? 'text-slate-700' : 'text-indigo-600'}`} />
                <span className="text-xs font-bold text-slate-700 line-clamp-1">{ch.posterName}</span>
              </div>
              <div className="flex items-center space-x-1.5 mt-0.5">
                {statusDot()}
                <span className="text-slate-300">•</span>
                <span className={`text-[11px] font-medium ${urgent ? 'text-red-600 font-bold' : 'text-slate-500'}`}>
        ...
        {/* Description */}
        <p className="text-xs text-slate-600 font-normal line-clamp-2 leading-relaxed mb-3 flex-1">
        ...
        <div className="flex flex-wrap gap-1.5 mb-4">
          ...
          {ch.tags.slice(0, 2).map(tag => (
            <span key={tag} className="text-[11px] px-2 py-0.5 rounded-full bg-slate-100 text-slate-700 border border-slate-300 font-medium">
              {tag}
            </span>
          ))}
          {ch.tags.length > 2 && <span className="text-[11px] text-slate-500 font-medium">+{ch.tags.length - 2}</span>}
        </div>
        ...
        <div className="pt-3 border-t border-slate-100 flex items-center justify-between">
          <div>
            ...
            <div className="flex items-center space-x-1 mt-0.5">
              ...
              <span className="text-[10px] text-green-700 font-bold">Escrow Locked</span>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-3 text-xs text-slate-600 font-medium">
            ...
            <span className="flex items-center space-x-1 text-blue-700 font-bold group-hover:translate-x-0.5 transition-transform">
              <span>View</span>
