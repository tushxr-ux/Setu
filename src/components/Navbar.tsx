...
export const Navbar: React.FC = () => {
  ...
  return (
    <header className="sticky top-0 z-40 w-full bg-white/95 backdrop-blur-md border-b border-slate-200 shadow-sm">
        ...
        <div className="flex items-center space-x-3">
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center space-x-2 bg-white border border-slate-200 hover:border-blue-300 rounded-xl px-3 py-2 text-sm transition-colors shadow-sm"
            >
      ...
      <div className="md:hidden flex border-t border-slate-100 overflow-x-auto no-scrollbar px-2 py-1.5 gap-1 text-xs">
        {[
          { tab: 'explore', label: 'Challenges' },
          { tab: 'leaderboard', label: 'Leaders' },
          ...(role === 'student' ? [{ tab: 'student-dash', label: 'My Work' }] : []),
          ...(role === 'company' || role === 'government' ? [{ tab: 'org-dashboard', label: `Inbox${pendingReviews > 0 ? ` (${pendingReviews})` : ''}` }, { tab: 'hire-pipeline', label: 'Talent' }] : []),
          ...(role === 'admin' ? [{ tab: 'admin-portal', label: 'Admin' }] : []),
        ].map(({ tab, label }) => (
          <button
            key={tab}
            onClick={() => setActiveTab(tab)}
            className={`px-3 py-1.5 rounded-lg whitespace-nowrap font-medium transition-colors ${activeTab === tab ? 'bg-blue-700 text-white' : 'text-slate-500'}`}
          >
            {label}
          </button>
        ))}
        {(role === 'company' || role === 'government') && (
          <button
            onClick={() => setIsPostModalOpen(true)}
            className="ml-auto px-3 py-1.5 rounded-lg bg-blue-700 text-white font-semibold whitespace-nowrap"
          >
            + Post
          </button>
        )}
